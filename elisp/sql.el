...
"value_error" "zero_divide") t) "\\b")))
	(oracle-reserved-combo (eval-when-compile
				 (concat "\\b"
					 (regexp-opt '("replace") t)
					 "\\b[ \t\r\n]*[^(]")))
	(oracle-reserved-words (eval-when-compile
				 (concat "\\b"
					 (regexp-opt '(
"access" "add" "alter" "audit" "bulk" "cluster" "column" "collect" "comment" "compress"
"connect" "drop" "else" "exclusive" "file" "forall" "grant" "heap"
"identified" "immediate" "increment" "index" "initial" "intersect"
"level" "lock" "long" "maxextents" "minus" "mode" "modify" "noaudit"
"nocompress" "nowait" "number" "object" "offline" "online" "organization" "pctfree" "prior"
"raw" "rename" "record" "ref" "resource" "revoke" "row" "rowlabel" "rownum"
"rows" "session" "share" "size" "start" "successful" "synonym" "sysdate"
"then" "trigger" "uid" "varying" "validate" "whenever") t) "\\b")))
	(oracle-types (eval-when-compile
			(concat "\\b"
				(regexp-opt '(
;; Oracle Keywords that look like types
;; Oracle Reserved Words that look like types
"array" "binary_integer" "blob" "boolean" "clob" "constant" "date" "decimal" "mlslabel" "rowid" "varchar" "varchar2") t) "\\b")))
	(oracle-dynatypes (eval-when-compile
			(concat "\\%\\("
				(regexp-opt '("rowtype" "type") t) "\\)\\b")))
	(oracle-builtin-dyn (eval-when-compile
			(concat "\\.\\("
				(regexp-opt '("exists" "count" "limit" "first" "last" "prior" "next" "extend" "trim" "delete") t) "\\)\\b")))
...
"translate" "trim" "trunc" "uid" "upper" "userenv" "variance" "vsize") t) "\\)[ \t\r\n]*[(]"))))
    (setq sql-mode-oracle-font-lock-keywords
	  (append sql-mode-ansi-font-lock-keywords
		  (list (cons oracle-keywords 'font-lock-function-name-face)
			...
			(cons oracle-reserved-combo 'font-lock-keyword-face)
			...
			(if (string-match "XEmacs\\|Lucid" emacs-version)
			  ...
			  (if (string-match "GNU Emacs 19" emacs-version)
			    ...
			    ;; Emacs
			    (list oracle-builtin-functions 1 'font-lock-builtin-face)
			    (list oracle-builtin-dyn 1 'font-lock-builtin-face)))
			(list oracle-dynatypes 1 'font-lock-type-face)
